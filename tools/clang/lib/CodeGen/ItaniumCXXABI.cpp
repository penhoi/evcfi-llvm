...
protected:
  ...
  bool VloomDisabled;
  // Three modes: VCFI_MODE=0, FLAG_MODE=1, POISON_MODE=2, INVALID=etc
  unsigned VloomMode;
  unsigned VloomSize;
  unsigned VloomRegs;
  unsigned nSanSeq;
...
public:
  ...
  void emitCXXRecordDervInfo(CodeGenVTables &CGVT,
                             const CXXRecordDecl *RD) override;
...
ItaniumCXXABI::GetVirtualBaseClassOffset(CodeGenFunction &CGF, Address This,
  ...
  VBaseOffsetPtr =
      CGF.Builder.CreateBitCast(VBaseOffsetPtr, CGM.PtrDiffTy->getPointerTo());
...
void ItaniumCXXABI::EmitDestructorCall(CodeGenFunction &CGF,
  ...
  if (getContext().getLangOpts().AppleKext && Type != Dtor_Base &&
    ...
    Callee = CGCallee::forDirect(
        CGM.getAddrOfCXXStructor(DD, getFromDtorType(Type)), DD);
...
#define DOLLAR_LEN 6
  ...
  start = 0;
  while ((end = str.find('$', start)) != StringRef::npos) {
...
void ItaniumCXXABI::emitCXXRecordDervInfo(CodeGenVTables &CGVT,
                                          const CXXRecordDecl *RD) {
  // llvm::GlobalVariable *VTable = getAddrOfVTable(RD, CharUnits());
  ItaniumVTableContext &VTContext = CGM.getItaniumVTableContext();
  const VTableLayout &VTLayout = VTContext.getVTableLayout(RD);

  // VLOOM class heirachy information.
  if (VloomDisabled)
    return;

  const VTableLayout::AddressPointsMapTy &addrPoints =
      VTLayout.getAddressPoints();
  for (auto i : addrPoints) {
    const CXXRecordDecl *Base = i.first.getBase();
    SmallString<256> vloomSym;
    SmallString<256> typ;
    vloomSym += "__VLOOM_VPTR_D";

    SmallString<256> derivedName;
    llvm::raw_svector_ostream StreamD(derivedName);
    getMangleContext().mangleCXXVTable(RD, StreamD);
    typ = derivedName;
    encode_dollar_symbol(typ);
    vloomSym += std::to_string(derivedName.size());
    vloomSym += typ;
    vloomSym += "_B";

    SmallString<256> baseName;
    llvm::raw_svector_ostream StreamB(baseName);
    getMangleContext().mangleCXXVTable(Base, StreamB);
    typ = baseName;
    encode_dollar_symbol(typ);
    vloomSym += std::to_string(baseName.size());
    vloomSym += typ;

    llvm::Constant *VPtr = getVTableAddressPoint(i.first, RD);
    size_t offset = 0;
    if (auto *GEP = llvm::dyn_cast<llvm::GEPOperator>(VPtr)) {
      llvm::APInt Offset(64, offset);
      if (GEP->accumulateConstantOffset(CGM.getDataLayout(), Offset))
        offset = Offset.getZExtValue();
      else {
        fprintf(stderr, "non-constant constant?\n");
        abort();
      }
    }
    vloomSym += "_O";
    vloomSym += std::to_string(offset);

    // llvm::GlobalVariable *VTable = getAddrOfVTable(RD, CharUnits());
    // fprintf(stderr, "%s = +%zu\n", vloomSym.str().str().c_str(), offset);
    std::string asmStr(".weak ");
    asmStr += vloomSym;
    asmStr += '\n';
    CGM.getModule().appendModuleInlineAsm(asmStr);
  }
}
...
llvm::GlobalVariable *ItaniumCXXABI::getAddrOfVTable(const CXXRecordDecl *RD,
  ...
  CGM.addCXXRecordVTable(RD); // record all vtables
...
void ItaniumCXXABI::initVloom(void) {
  ...
  if (vstr != NULL)
    VloomSize = atoi(vstr);
  assert(VloomSize >= 8 && VloomSize < 256);
...
CGCallee ItaniumCXXABI::getVirtualFunctionPointer(CodeGenFunction &CGF,
                                                  ...
                                                  llvm::Type *FTy,
                                                  SourceLocation Loc) {
  llvm::Type *Ty = FTy->getPointerTo()->getPointerTo();
  ...
  if (CGF.ShouldEmitVTableTypeCheckedLoad(MethodDecl->getParent())) {
    ...
    if (!gbVloomInited && !VloomDisabled) {
      gbVloomInited = true;
    ...
    if (!VloomDisabled) {
      // 1. call __VLOOM_SANITIZE__PH__ZTV15FormatterToText
      SmallString<256> strClassName;
      llvm::raw_svector_ostream Stream(strClassName);
      getMangleContext().mangleCXXVTable(MethodDecl->getParent(), Stream);
...
static void emitGlobalDtorWithCXAAtExit(CodeGenFunction &CGF,
  ...
  llvm::Value *args[] = {llvm::ConstantExpr::getBitCast(dtor, dtorTy),
...
llvm::Function *ItaniumCXXABI::getOrCreateThreadLocalWrapper(const VarDecl *VD,
  ...
  if (!Wrapper->hasLocalLinkage() &&
      !(isThreadWrapperReplaceable(VD, CGM) &&
...
ItaniumRTTIBuilder::GetAddrOfExternalRTTIDescriptor(QualType Ty) {
  ...
  if (!GV) {
    ...
    GV = new llvm::GlobalVariable(
        ...
        /*Constant=*/true, llvm::GlobalValue::ExternalLinkage, nullptr, Name);
