...
void CodeGenVTables::GenerateDervInfo(const CXXRecordDecl *RD) {
  CGM.getCXXABI().emitCXXRecordDervInfo(*this, RD);
}
...
void CodeGenModule::EmitCXXRecordDervInfo() {
  for (const CXXRecordDecl *RD : CXXRecordVTables) {
    VTables.GenerateDervInfo(RD);
  }
  CXXRecordVTables.clear();
}
